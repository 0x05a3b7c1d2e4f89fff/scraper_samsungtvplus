...
# Prefix applied to all channel IDs so M3U tvg-id and EPG channel/programme
# attributes stay in sync after Samsung changed their raw IDs.
CHANNEL_ID_PREFIX = os.getenv('CHANNEL_ID_PREFIX', 'samsung-')

def rewrite_epg_ids(epg_bytes, prefix):
    """
    Patch raw EPG XML so every channel id= and programme channel= attribute
    gains the same prefix that was applied to tvg-id in the M3U playlist.

    Works on the raw bytes with a simple string replace so we never need to
    parse the potentially huge XML document.
    """
    if not prefix:
        return epg_bytes

    text = epg_bytes.decode('utf-8')

    import re

    # <channel id="US390001557"> → <channel id="samsung-US390001557">
    # Guard against double-prefixing on re-runs.
    text = re.sub(
        r'(<channel\s+id=")(?!' + re.escape(prefix) + r')',
        r'\g<1>' + prefix,
        text,
    )

    # <programme channel="US390001557" ...> → <programme channel="samsung-US390001557" ...>
    text = re.sub(
        r'(<programme\b[^>]*?\bchannel=")(?!' + re.escape(prefix) + r')',
        r'\g<1>' + prefix,
        text,
    )
...
def generate_m3u_playlist(data, channels):
    ...
    for channel_id, channel in sorted_channels:
        ...
        # Generate M3U entry
        prefixed_id = f"{CHANNEL_ID_PREFIX}{channel_id}" if CHANNEL_ID_PREFIX else channel_id
        extinf_line = f'#EXTINF:-1 tvg-id="{prefixed_id}" tvg-name="{name}" tvg-logo="{logo}" group-title="{group}" tvg-chno="{chno}"'
...
def main():
    ...
    try:
        ...
        epg_content = rewrite_epg_ids(epg_content, CHANNEL_ID_PREFIX)
        if CHANNEL_ID_PREFIX:
            print(f"Rewrote EPG channel IDs with prefix '{CHANNEL_ID_PREFIX}'")
